// Desktop JS surface vendored from Deno 2.9.7 `cli/rt/desktop.rs`
// (MIT, Copyright 2018-2026 the Deno authors). Kept byte-for-byte except for
// `serde_json` -> `deno_core::serde_json`. This anchors desktop API parity to
// the `deno_runtime`/`laufey` versions inka pins.
#![allow(clippy::all)]
#![allow(dead_code)]
...
"#,
        version = deno_core::serde_json::to_string(&version).unwrap(),
        rolled_back = if rolled_back { "true" } else { "false" },
        release_base_url = deno_core::serde_json::to_string(&release_base_url).unwrap(),
    )
}
...
pub fn desktop_error_reporting_js(url: Option<&str>, version: Option<&str>) -> String {
    format!(
        r#"(() => {{
...
"#,
        url = deno_core::serde_json::to_string(&url).unwrap(),
        version = deno_core::serde_json::to_string(&version).unwrap(),
    )
}
