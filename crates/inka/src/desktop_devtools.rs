...
/// `anyhow::Error`, aliased to match the vendored Deno source.
type AnyError = anyhow::Error;

/// Bind an ephemeral loopback port and return it. Replaces `deno_lib`'s helper.
pub fn allocate_random_port() -> Result<u16, AnyError> {
    let listener = std::net::TcpListener::bind(("127.0.0.1", 0))?;
    Ok(listener.local_addr()?.port())
}

/// A validated `Host` header, restricted to localhost/loopback authorities so
/// a page in the app can't drive the mux via DNS-rebinding.
#[derive(Clone, Debug)]
...
/// Split `host[:port]` (including `[::1]:port`).
...
/// Validate the request `Host` header (loopback hosts only). A missing header
/// is allowed (DevTools clients sometimes omit it).
fn validated_host_header<T>(req: &hyper::Request<T>) -> Result<Option<ValidatedHost>, ()> {
    let Some(value) = req.headers().get(http::header::HOST) else {
        return Ok(None);
    };
    let authority = value.to_str().map_err(|_| ())?.to_string();
    let (hostname, port) = split_host_port(&authority).ok_or(())?;
    let loopback =
        hostname.eq_ignore_ascii_case("localhost") || hostname == "127.0.0.1" || hostname == "::1";
    if !loopback {
        return Err(());
    }
    Ok(Some(ValidatedHost {
        authority,
        hostname,
        port,
    }))
...
pub async fn spawn_mux(config: MuxConfig) -> Result<MuxHandle, AnyError> {
    let listener = TcpListener::bind(config.listen)
        .await
        .with_context(|| format!("failed to bind CDP multiplexer to {}", config.listen))?;
...
async fn serve_connection(stream: TcpStream, state: Arc<MuxState>) -> Result<(), AnyError> {
...
async fn handle_request(
    ...
    let host = match validated_host_header(&req) {
        Ok(host) => host,
        Err(_) => {
            return simple_response(http::StatusCode::BAD_REQUEST, "Invalid Host header");
        }
    };
    if req.method() != http::Method::GET {
        return simple_response(http::StatusCode::METHOD_NOT_ALLOWED, "Not Allowed");
    }
...
fn valid_ws_origin<T>(req: &hyper::Request<T>, host: Option<&ValidatedHost>) -> bool {
...
fn json_version(state: &MuxState, host: Option<&ValidatedHost>) -> hyper::Response<Full<Bytes>> {
    let authority = host
...
fn json_list(state: &MuxState, host: Option<&ValidatedHost>) -> hyper::Response<Full<Bytes>> {
    let authority = host
        .map(ValidatedHost::authority)
        .map(String::from)
        .unwrap_or_else(|| state.listen.to_string());
...
fn simple_response(status: http::StatusCode, msg: &'static str) -> hyper::Response<Full<Bytes>> {
    hyper::Response::builder()
        .status(status)
        ...
        match kind {
            ...
            TargetKind::Deno => {
                ...
                match connect_upstream(&state, kind).await {
                    ...
                    Err(err) => {
                        log::error!("[devtools-mux] failed to connect upstream for Deno: {err:?}");
                    }
                }
            }
            TargetKind::Cef => match connect_upstream(&state, kind).await {
                Ok(mut upstream) => {
                    if state.config.inspect_brk {
                        if let Err(err) = inject_cef_pause(&mut upstream).await {
                            log::error!("[devtools-mux] failed to inject pause into CEF: {err:?}");
                        }
                    }
                    mark_debugger_attached(&state);
                    if let Err(err) = proxy_frames(client, upstream).await {
                ...
                Err(err) => {
                    log::error!("[devtools-mux] failed to connect upstream for CEF: {err:?}");
                }
            },
        }
    });
...
async fn fetch_upstream_ws_url(host: SocketAddr) -> Result<String, AnyError> {
    ...
    let ws_url = value
        ...
        .and_then(|v| v.as_str())
        .ok_or_else(|| anyhow!("no webSocketDebuggerUrl in /json/list at {host}"))?;
    ...
    S: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin,
{
    ...
    log::debug!("[devtools-mux] injected Debugger.enable + Debugger.pause into CEF");
    Ok(())
}
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                let owned = if opcode == OpCode::Text {
                    OwnedFrame::text(rewrite_text_from_upstream(&payload, None, "Renderer"))
                } else {
...
fn route_client_text(
    ...
    ) && !deno_announced.swap(true, std::sync::atomic::Ordering::SeqCst)
    {
        ...
        let _ = client_send.send(OwnedFrame::text(serde_json::to_vec(&event).unwrap()));
    }
    ...
    if method.as_deref() == Some("Target.attachToTarget") {
        ...
        if target_id == Some(DENO_CHILD_TARGET_ID) {
            if let Some(rid) = id {
                ...
                let _ = client_send.send(OwnedFrame::text(serde_json::to_vec(&reply).unwrap()));
            }
            return;
        }
    }
...
fn rewrite_text_from_upstream(
    ...
    let is_context_created =
        obj.get("method").and_then(|v| v.as_str()) == Some("Runtime.executionContextCreated");
...
mod tests {
    ...
    fn request_with_host_and_origin(host: Option<&str>, origin: Option<&str>) -> http::Request<()> {
    ...
    fn websocket_origins_match_client_visible_authority() {
        for origin in [
            ...
            let request = request_with_host_and_origin(Some("localhost:43123"), origin);
        ...
        for origin in [
            ...
            let request = request_with_host_and_origin(Some("localhost:43123"), Some(origin));
            let host = validated_host_header(&request).unwrap();
            assert!(!valid_ws_origin(&request, host.as_ref()));
        }

        let request = request_with_host_and_origin(None, Some("http://localhost:43123"));
    ...
    fn route_client_text_lazily_announces_deno() {
        ...
        route_client_text(&payload, "sess", &client_tx, &cef_tx, &deno_tx, &announced);
    ...
    async fn json_list_returns_three_targets() {
        ...
        assert!(list[0]["webSocketDebuggerUrl"]
            .as_str()
            .unwrap()
            .ends_with("/unified"));
        ...
        assert!(list[1]["webSocketDebuggerUrl"]
            .as_str()
            .unwrap()
            .ends_with("/deno"));
        ...
        assert!(list[2]["webSocketDebuggerUrl"]
            .as_str()
            .unwrap()
            .ends_with("/cef"));
        ...
        for page_entry in [&list[0], &list[2]] {
            assert!(page_entry["devtoolsFrontendUrl"]
                .as_str()
                .unwrap()
    ...
    async fn test_connect_ws(ws_url: &str) -> WebSocket<TokioIo<hyper::upgrade::Upgraded>> {
    ...
    async fn integration_http_endpoints() {
        ...
        let (status, body) = http_get_with_host(mux.listen, "/json/list", "localhost:43123").await;
        assert_eq!(status, http::StatusCode::OK);
        let list: Vec<Value> = serde_json::from_slice(&body).unwrap();
        assert_eq!(
        ...
        let (status, _) = http_get_with_host(mux.listen, "/json/list", "example.test:43123").await;
    ...
    async fn integration_unified_session_routes_and_rewrites() {
        ...
        let received_val: Value = serde_json::from_slice(&received.payload).unwrap();
        ...
        let deno_val: Value = serde_json::from_slice(&deno_received.payload).unwrap();
    ...
    async fn integration_inspect_brk_injects_before_marking_attached() {
        ...
        let ws_url = format!("ws://{}/unified", mux.listen);
        let client_connect = tokio::spawn(async move { test_connect_ws(&ws_url).await });
        ...
        let enable_val: Value = serde_json::from_slice(&enable_frame.payload).unwrap();
        ...
        let pause_val: Value = serde_json::from_slice(&pause_frame.payload).unwrap();
