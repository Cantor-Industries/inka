...
1. parses its own trailer (an `INKFOOT5` bundle + embedded files),
...
3. dlopens it and calls the frozen C ABI (`inka_runtime_create` /
   `inka_runtime_destroy` / `inka_runtime_run_module_dir`) with the payload,
   argv, and the manifest's permission DSL,
4. fails closed (exit 4) if the installed runtime lacks the `_dir` entry point.
   There is no permission-less fallback, so deny-by-default can never degrade
   to allow-all.
